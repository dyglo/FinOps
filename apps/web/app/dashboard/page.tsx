    ...
    <div className="flex flex-col gap-6 py-4 pb-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
           <div className="h-10 w-10 rounded-xl bg-black flex items-center justify-center shadow-lg shadow-black/20">
              <Terminal className="h-5 w-5 text-white" />
           </div>
           <div>
              <h1 className="text-xl font-bold text-black tracking-tight">FinOps Terminal</h1>
              <div className="flex items-center gap-2">
                 <Badge variant="success" className="text-[8px] h-4 px-1.5 font-black uppercase">System Live</Badge>
                 <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">v2.0.4-alpha</span>
              </div>
           </div>
        </div>
        ...
        <div className="flex-1 max-w-xl relative">
           ...
           <Input 
             ...
             className="pl-10 h-10 bg-white/40 border-gray-200 rounded-xl glass-card border-none shadow-sm focus-visible:ring-black"
           />
        </div>

        <div className="flex items-center gap-3">
           <div className="flex flex-col items-end px-3 border-r border-gray-100">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">AI Latency</span>
              <span className="text-xs font-bold text-emerald-600">12ms</span>
           </div>
           <div className="flex flex-col items-end px-3">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">API Health</span>
              <span className="text-xs font-bold text-blue-600">99.9%</span>
           </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-6 h-full">
         ...
         <div className="lg:col-span-8 space-y-6">
            ...
            <div className="flex items-center justify-between px-2">
               <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-[0.2em] text-gray-500">Intelligence Workspace</h2>
               </div>
               <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-[10px] h-6 px-3 rounded-md bg-white border-gray-100 font-bold uppercase text-gray-400">Real-time Feed</Badge>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400"><ListFilter className="h-3 w-3" /></Button>
               </div>
            </div>
            ...
            <GlassCard className="p-0 overflow-hidden min-h-[500px] border-none shadow-xl">
               <div className="p-5 border-b border-gray-100 bg-white/60 backdrop-blur-md flex items-center justify-between">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-sm font-bold text-black">Live Signals Ledger</span>
                  </div>
                  <div className="flex items-center gap-2">
                     <Badge className="text-[9px] bg-black text-white border-none h-5 font-black uppercase tracking-widest px-2">42 New Signals</Badge>
                  </div>
               </div>
               <div className="divide-y divide-gray-50 overflow-y-auto max-h-[600px] custom-scrollbar">
         ...
         <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="flex items-center gap-2 px-2">
               ...
               <h2 className="text-sm font-black uppercase tracking-[0.2em] text-gray-500">Verified Intelligence</h2>
            </div>
...
function MetricItem({ label, value, change, icon }: any) {
  return (
    <GlassCard className="p-4 flex flex-col gap-1 hover:bg-white transition-all group border-none shadow-sm cursor-pointer">
       <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter group-hover:text-black transition-colors">{label}</span>
          {icon}
       </div>
       <div className="flex items-baseline gap-2">
          <span className="text-xl font-bold text-black font-mono tracking-tighter">{value}</span>
...
function SignalRow({ ticker, signal, impact, confidence, age, color }: any) {
  ...
  return (
    <div className="p-5 flex items-center justify-between hover:bg-gray-50/50 transition-colors group cursor-pointer">
       <div className="flex items-center gap-4">
          <div className={`h-2 w-2 rounded-full ${colorMap[color]} shadow-sm group-hover:scale-125 transition-transform`} />
          <div className="flex flex-col">
             <span className="text-xs font-black text-black">{ticker}</span>
             <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">{signal}</span>
          </div>
       </div>
       <div className="flex items-center gap-10 text-right">
          <div className="flex flex-col">
             <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Impact</span>
             <span className={`text-[10px] font-bold ${impact === 'High' ? 'text-black' : 'text-gray-500'}`}>{impact}</span>
          </div>
          <div className="flex flex-col w-12">
             <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Confidence</span>
             <span className="text-[10px] font-bold text-black">{confidence}</span>
          </div>
          <span className="text-[10px] font-bold text-gray-300 w-16">{age}</span>
       </div>
    </div>
  )
}
